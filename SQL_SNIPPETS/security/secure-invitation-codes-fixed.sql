...
-- Create a secure function to access invitation code usage (admin only)
...
-- Note: We don't enable RLS on views as it's not supported
CREATE OR REPLACE VIEW public.invitation_code_usage AS
SELECT * FROM public.get_invitation_code_usage();

-- Restrict access to the view using grants
REVOKE ALL ON public.invitation_code_usage FROM PUBLIC;
GRANT SELECT ON public.invitation_code_usage TO authenticated;

-- Create an admin function to check if the current user is an admin
CREATE OR REPLACE FUNCTION public.is_admin()
RETURNS BOOLEAN AS $$
DECLARE
  v_is_admin BOOLEAN;
BEGIN
  -- Check if user has admin role in user_settings or another appropriate table
  -- This is a placeholder - implement based on your actual admin role storage
  SELECT EXISTS (
